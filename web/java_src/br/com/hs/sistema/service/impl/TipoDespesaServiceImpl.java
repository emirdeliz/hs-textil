package br.com.hs.sistema.service.impl;

import java.io.Serializable;
import java.util.List;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.flex.remoting.RemotingDestination;
import org.springframework.stereotype.Service;

import br.com.hs.model.entity.TipoDespesa;
import br.com.hs.sistema.controller.logger;
import br.com.hs.sistema.repository.TipoDespesaDao;
import br.com.hs.sistema.service.TipoDespesaService;

@Service
@RemotingDestination(value="tipoDespesaService")
public class TipoDespesaServiceImpl implements TipoDespesaService{
	
	@Autowired	
	private TipoDespesaDao repository;
		
	public TipoDespesa buscarPorId(Serializable oid)throws Exception {
		// TODO Auto-generated method stub
		try {
			logger.info("List entity TipoDespesa by id: " + oid);	
			return this.repository.buscarPorId(TipoDespesa.class, oid);
		} catch (Exception e) {
			// TODO: handle exception
			logger.error("An error occurred while querying data: " + e);
			throw e;
		}
	}
	
	public List<TipoDespesa> buscarTodos()throws Exception {
		// TODO Auto-generated method stub
		try {
			logger.info("List All entity: TipoDespesa");
			return this.repository.buscarTodos(TipoDespesa.class);	
		} catch (Exception e) {
			// TODO: handle exception
			logger.error("An error occurred while all query data: " + e);
			throw e;
		}	
	}
	
	public void deletar(TipoDespesa entity)throws Exception {
		// TODO Auto-generated method stub		
		try {
			logger.info("Delete a new entity: " + entity);
			this.repository.deletar(entity);
		}catch (Exception e) {
			// TODO: handle exception
			logger.error("An error occurred while deleting item: " + e);
			throw e;
		}
	}
	
	public TipoDespesa salvar(TipoDespesa entity)throws Exception {
		// TODO Auto-generated method stub
		try {			
			logger.info("Save a new entity: " + entity);
			logger.info((entity.getOid()>0? "updating -->":"saving -->"));
			return entity.getOid()>0?repository.atualizar(entity):repository.salvar(entity);
		}catch (Exception e) {
			// TODO: handle exception
			logger.info("An error occurred while: " + (entity.getOid()>0? "updating":"saving") + " item: " + e);
			throw e;
		}			 
	}	
	
	@Override
	public List<TipoDespesa> buscarTodosAtivos() throws Exception {
		try {
			logger.info("List All ativos entity: TipoDespesa");
			return this.repository.buscarTodosAtivos(TipoDespesa.class);	
		} catch (Exception e) {
			// TODO: handle exception
			logger.error("An error occurred while all query data: " + e);
			throw e;
		}	
	}

}
